...
const OptionManagementScreen = ({ screenChange }: Props) => {
  ...
  const loadOptions = async () => {
    try {
      const response = await axios.get(`http://192.168.0.191:8080/api/options/list/${ownerId}`, {
        headers: {
  ...
  const addOption = async () => {
    ...
    try {
      await axios.post(`http://192.168.0.191:8080/api/options/add?ownerId=${ownerId}&optionName=${encodeURIComponent(newOptionName)}`, {}, {
        headers: {
          'Authorization': `Bearer ${await AsyncStorage.getItem('jwtToken')}`,
          'accept': '*/*'
        }
  ...
  const deleteOption = async (optionId: string) => {
    try {
      await axios.delete(`http://192.168.0.191:8080/api/options/delete/${optionId}`, {
        headers: {
          'Authorization': `Bearer ${await AsyncStorage.getItem('jwtToken')}`,
          'accept': '*/*'
        }
